class Controller {
  ...
  handlerViewTextTopChange = (text, error) => { //написали новый текст1, если он меньше 140 символов, то передай в модель текст, модель изменит сеттером, передаст в контроллер изменения и они запишутс в превью
        ...
        if (text.length > 20) {  //если больше чем 140 символов, то скомандуй модели  изменить , потом она передаст изменения в контроллер, и контроллер скомандует вью написать сообщение об ошибке
  ...
  handlerViewTextBottomChange = (text, error) => {
    if (text.length > 20) {  //если больше чем 140 символов, то скомандуй модели  изменить состояние на true, потом она передаст изменения в контроллер, и контроллер скомандует вью написать сообщение об ошибке
